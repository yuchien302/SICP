...
;; parameters (if any) will have as values the initial arguments, as usual,
...
(define (same-parity x . y)
  (define (same-parity-iter party l res)
    (cond ((null? l) (reverse res))   ;; reverse the final list
          ((eq? (even? party) (even? (car l)))
           (same-parity-iter party (cdr l) (cons (car l) res)))
          (else (same-parity-iter party (cdr l) res))))
  (same-parity-iter x y (list x)))
